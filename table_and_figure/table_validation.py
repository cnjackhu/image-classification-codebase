...
def df_to_latex_with_bold(df: pd.DataFrame, metric_pairs: list) -> str:
    ...
    for base_col, best_col in metric_pairs:
        ...
        latex_df.loc[mask, best_col] = latex_df.loc[mask, best_col].apply(
            lambda x: f"\\textbf{{{x}}}"
        )
        latex_df.loc[~mask, base_col] = latex_df.loc[~mask, base_col].apply(
            lambda x: f"\\textbf{{{x}}}"
        )
    ...
    latex_table = latex_table.replace(
        "\\begin{tabular}{" + "r" * len(df.columns) + "}\n", ""
    )
...
def prepare_filtered_df(
    df_base: pd.DataFrame, df_best: pd.DataFrame, output_filename: str
) -> pd.DataFrame:
    ...
    # Target folder inside the project
    tables_dir = os.path.join(BASE_DIR, "tables")
    os.makedirs(tables_dir, exist_ok=True)
    output_path = os.path.join(tables_dir, f"{output_filename}_big.tex")
    ...
    output_path = os.path.join(tables_dir, f"{output_filename}_small.tex")
    with open(output_path, "w") as f:
        f.write(latex_table)
...
def create_summary_latex_table(
    comparison_results: dict, dataset_name: str, lambda_values: list
) -> str:
    """
...
def process_and_compare_summary(file_path: str, name: str, celoss: int):
    ...
    summary_table = create_summary_latex_table(
        comparison_results, name.upper(), lambda_values
    )
        # Directory of the current script
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    # Target folder inside the project
    tables_dir = os.path.join(BASE_DIR, "tables")
    os.makedirs(tables_dir, exist_ok=True)
    output_path = os.path.join(tables_dir, f"{name}_lambda_summary.tex")
    ...
    output_path = os.path.join(tables_dir, f"{name}_summary.tex")
    with open(output_path, "w") as f:
        f.write(summary_table)
    print("\nSummary table content:")
    print(summary_table)
