...
def update_metrics(metrics_dic, model, loader, train_loss, loss_fn=nn.CrossEntropyLoss(reduction="none")):
    ...
    """
    (log_loss, zero_one_losses),ece,mce = get_losses(model, loader, loss_fn=[loss_fn, zero_one_loss])
    ...
    metrics_dic['ece'].append(ece)
    metrics_dic['mce'].append(mce)
...
def save_model(model, args, folder_name='./checkpoint/bench/', prefix=''):
    # Get the argument configuration
    arg_config = get_argument_config()
    
    # Start with prefix 
    filename_parts = [f"{prefix}"]
    ...
    # Join all parts and add extension
    metrics_filename = "".join(filename_parts)
    
    os.makedirs(folder_name, exist_ok=True)
    filename = f'{folder_name}/{metrics_filename}.pth'
